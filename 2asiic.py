...
def process_image(image_path, output_width=100):
    try:
        # 使用PIL打开图像以支持更多格式
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='将图像/视频/GIF转换为ASCII艺术')
    parser.add_argument('input_file', type=str, help='输入文件路径(图像、视频或GIF)')
    parser.add_argument('--width', type=int, default=100, help='输出宽度（字符数）')
    parser.add_argument('--fps', type=int, default=30, help='最大FPS限制（仅对视频/GIF有效）')
    ...
    elif args.input_file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
        process_image(args.input_file, args.width)
    else:
